(()=>{
  ...
  const safeSet=(key,value)=>{
    try{localStorage.setItem(key,value);return true}
      ...
      if(confirm('Penyimpanan browser penuh. Hapus data PAIBP versi lama agar V23.1 dapat dijalankan? File aplikasi lama di komputer tetap aman.')){
        cleanupOldMath();
  ...
  if(!store||!Array.isArray(store.orders)||!store.orders.length){
    ...
    try{profile={...profile,...JSON.parse(localStorage.getItem('eperangkat.pendidikan-agama-islam.faseb.v1.profile')||'{}')}}catch(_){}
    let students=[];
    try{students=JSON.parse(localStorage.getItem('eperangkat.pendidikan-agama-islam.faseb.v1.students')||'[]')}catch(_){}
    const grade=localStorage.getItem('eperangkat.pendidikan-agama-islam.faseb.v1.grade')||'III';
  ...
  active.grade=active.grade||'III';
  safeSet(KEY,JSON.stringify(store));
  safeSet('eperangkat.pendidikan-agama-islam.faseb.v1.profile',JSON.stringify(lightProfile(active.profile)));
  safeSet('eperangkat.pendidikan-agama-islam.faseb.v1.students',JSON.stringify(active.students));
  safeSet('eperangkat.pendidikan-agama-islam.faseb.v1.grade',active.grade);
})();
