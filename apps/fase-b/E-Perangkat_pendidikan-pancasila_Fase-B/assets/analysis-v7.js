(()=>{
  ...
  const ORDER_KEY='eperangkat.pendidikan-pancasila.faseb.v1.orders';
  const VIEW_KEY='eperangkat.pendidikan-pancasila.faseb.v1.view';
  const GRADE_KEY='eperangkat.pendidikan-pancasila.faseb.v1.grade';
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>[...r.querySelectorAll(s)];
  const clone=x=>JSON.parse(JSON.stringify(x));
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const gradeNumber={I:1,II:2,III:3,IV:4,V:5,VI:6,VII:7,VIII:8,IX:9,X:10,XI:11,XII:12};
  let currentSemester=localStorage.getItem('eperangkat.pendidikan-pancasila.faseb.v1.analysis.semester')||'1';
  let draftOrder=null;
  ...
  function renderAnalysis(){
    ...
    localStorage.setItem('eperangkat.pendidikan-pancasila.faseb.v1.analysis.semester',currentSemester);
  ...
  function bindAnalysisEvents(){
    $$('[data-analysis-semester]').forEach(btn=>btn.onclick=()=>{currentSemester=btn.dataset.analysisSemester;localStorage.setItem('eperangkat.pendidikan-pancasila.faseb.v1.analysis.semester',currentSemester);renderAnalysis()});
  ...
  function persistDraft(action,detail){
    ...
    setStore(store);
    if(store.activeId===live.id)localStorage.setItem('eperangkat.pendidikan-pancasila.faseb.v1.students',JSON.stringify(live.students));
