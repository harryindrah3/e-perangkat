(()=>{
  ...
  const PROFILE_KEY='eperangkat.pendidikan-agama-islam.fased.v1.profile';
  ...
  function rowHtml(row,index){
    return `<tr data-schedule-index="${index}">
      ...
      <td><input data-field="className" value="${esc(row.className)}" placeholder="VII"></td>
  ...
  function render(){
    const app=$('#app');
    if(!app)return;
    const p=profile(),grade=localStorage.getItem(GRADE_KEY)||'VII';
    ...
    app.innerHTML=`<div class="page-head"><div><span class="eyebrow">Kelas ${esc(grade)} · Fase D</span>
    ...
    $('#resetScheduleExample').onclick=()=>{
      ...
      rows.push(
        {semester:'1',className:'VII',day:'SENIN',time:'08.00 – 10.15',hours:3},
        {semester:'2',className:'VII',day:'SENIN',time:'08.00 – 10.15',hours:3}
      );
