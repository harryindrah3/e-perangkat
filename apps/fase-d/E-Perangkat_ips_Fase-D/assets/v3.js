(()=>{
  ...
  const KEY='eperangkat.ips.fased.v1.orders';
  ...
  function storageBytes(){
    ...
    for(let index=0;index<localStorage.length;index++){
      ...
      total+=size;
      if(key.startsWith('eperangkat.ips.fased.v1.'))current+=size;
      else if(key.startsWith('eperangkat.ips.fased.v1.legacy.')&&!key.startsWith('eperangkat.ips.fased.v1.'))oldMath+=size;
    }
  ...
  function obsoleteMathKeys(){
    ...
    for(let index=0;index<localStorage.length;index++){
      const key=localStorage.key(index)||'';
      if(key.startsWith('eperangkat.ips.fased.v1.legacy.')&&!key.startsWith('eperangkat.ips.fased.v1.'))keys.push(key);
    }
    return keys;
  }
  ...
  function storageStatusHtml(){
    ...
    return `<div class="storage-status-panel"><div><b>Penggunaan penyimpanan browser</b><span>Total ${formatBytes(size.total)} · V23.1 ${formatBytes(size.current)} · IPS versi lama ${formatBytes(size.oldMath)}</span></div><div><b>${count}</b><span>kunci data IPS versi lama</span></div></div>`;
  }
  ...
  function setStore(store){
    ...
    if(oldKeys.length&&confirm(`Penyimpanan browser penuh. Ditemukan ${oldKeys.length} data IPS dari versi lama. Hapus data browser versi lama agar penyimpanan tersedia? File aplikasi lama tetap aman di komputer.`)){
    ...
    const storageError=new Error('Penyimpanan browser penuh. Gunakan tombol “Bersihkan Data IPS Lama”, lalu ulangi impor.');
  ...
  function saveAndSync(store,order,{reload=false}={}){
    ...
    if(active){
      localStorage.setItem('eperangkat.ips.fased.v1.profile',JSON.stringify(lightProfile(active.profile||D.defaults)));
      localStorage.setItem('eperangkat.ips.fased.v1.students',JSON.stringify(active.students||[]));
      localStorage.setItem('eperangkat.ips.fased.v1.grade',active.grade||'VII');
    }
    refreshOrderBar();
  ...
  function pageHead(title,description,actions=''){
    const store=getStore(),order=activeOrder(store),grade=order?.grade||localStorage.getItem('eperangkat.ips.fased.v1.grade')||'VII';
  ...
  function bindOrderEvents(){
    ...
    $('#exportOrdersBtn').onclick=async()=>{
      ...
      a.download=`backup-eperangkat-IPS-v23-1-${new Date().toISOString().slice(0,10)}.json`;
      a.click();
      ...
      reader.onload=async()=>{
        try{
          ...
          try{
            ...
            const oldKeys=obsoleteMathKeys();
            if(oldKeys.length&&!confirm(`Backup sudah dipadatkan, tetapi ruang masih kurang. Hapus ${oldKeys.length} data browser IPS versi lama lalu lanjutkan impor?`)){
    ...
    $('#cleanupOldMathBtn').onclick=()=>{
      const keys=obsoleteMathKeys();
      if(!keys.length){alert('Tidak ada data IPS versi lama di penyimpanan browser.');return}
      if(!confirm(`Hapus ${keys.length} data browser IPS versi lama? File ZIP dan folder aplikasi lama di komputer tidak ikut terhapus.`))return;
      cleanupOldMathStorage();
      alert('Data browser IPS versi lama berhasil dibersihkan.');
      renderOrders();
    };
  }
  ...
  function renderCalendar(){
    ...
    app.innerHTML=pageHead('Kalender Pendidikan','Kalender Pendidikan: MES 24, MEB 21, HES 112 hari, dan HEB 100 hari menjadi dasar Program Tahunan serta Program Semester IPS.',`<a class="btn primary" href="print.html?grade=${o.grade}&section=calendar&semester=${sem}">Cetak Semester ${sem===1?'I':'II'}</a><a class="btn secondary" href="print.html?grade=${o.grade}&section=calendar&semester=both">Cetak Keduanya</a>`)+`
  ...
  function currentCustomView(){return localStorage.getItem('eperangkat.ips.fased.v1.view')}
  ...
  $$('[data-grade]').forEach(b=>b.addEventListener('click',()=>setTimeout(()=>{
    ...
    const s=getStore(),o=activeOrder(s),g=localStorage.getItem('eperangkat.ips.fased.v1.grade')||b.dataset.grade;if(o&&o.grade!==g){const before=snapshot(o);o.grade=g;history(o,'Kelas pesanan diubah',`Kelas diubah menjadi ${g}.`,before);setStore(s)}
  },0)));
  ...
  $('#quickPrint')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openPrint(`print.html?grade=${localStorage.getItem('eperangkat.ips.fased.v1.grade')||'VII'}&section=all`,'auto')},true);
  ...
  document.addEventListener('click',e=>{
    if(e.target.id==='saveProfile')setTimeout(()=>{const s=getStore(),o=activeOrder(s);if(!o)return;let p={...D.defaults};try{p={...p,...JSON.parse(localStorage.getItem('eperangkat.ips.fased.v1.profile')||'{}')}}catch(_){}o.profile=p;o.school=p.school;history(o,'Profil satuan pendidikan diperbarui','Identitas dokumen disimpan melalui menu Profil.',beforeProfileSnapshot);setStore(s);refreshOrderBar()},20);
    if(e.target.id==='saveStudents')setTimeout(()=>{const s=getStore(),o=activeOrder(s);if(!o)return;const before=snapshot(o);try{o.students=JSON.parse(localStorage.getItem('eperangkat.ips.fased.v1.students')||'[]')}catch(_){o.students=[]}history(o,'Data peserta dan nilai diperbarui',`${o.students.length} peserta tersimpan.`,before);setStore(s)},20);
  });

  // Tombol profil diarahkan ke profil milik pesanan aktif, bukan profil global terpisah.
  $('#profileBtn')?.addEventListener('click',e=>{
    e.preventDefault();e.stopImmediatePropagation();
    localStorage.setItem('eperangkat.ips.fased.v1.view','orders');
  ...
  window.addEventListener('eperangkat:students-updated',event=>{
    ...
    try{o.students=JSON.parse(localStorage.getItem('eperangkat.ips.fased.v1.students')||'[]')}
    catch(_){o.students=[]}
