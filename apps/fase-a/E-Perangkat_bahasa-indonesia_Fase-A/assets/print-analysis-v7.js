(()=>{
  ...
  const STORE='eperangkat.bahasa-indonesia.fasea.v1.orders';
  ...
  const grade=params.get('grade')||order.grade||localStorage.getItem('eperangkat.bahasa-indonesia.fasea.v1.grade')||'I';
  ...
  function cover(sem){return `<section class="page portrait cover analysis-cover"><div class="page-inner cover-content"><div class="cover-kicker">KURIKULUM MERDEKA · PEMBELAJARAN MENDALAM</div><div class="cover-center"><small>E-PERANGKAT PEMBELAJARAN</small><h1>ANALISIS NILAI FORMATIF DAN SUMATIF</h1><h2>B. Indonesia · Kelas ${className} · Fase A · Semester ${sem==='1'?'I (Ganjil)':'II (Genap)'}</h2><div class="accent-line"></div><div class="identity-box"><table><tr><td>Satuan Pendidikan</td><td>:</td><td><b>${esc(profile.school)}</b></td></tr><tr><td>Program</td><td>:</td><td><b>${esc(profile.program)}</b></td></tr><tr><td>${esc(teacherRole)}</td><td>:</td><td><b>${esc(profile.teacher)}</b></td></tr><tr><td>Tahun Pelajaran</td><td>:</td><td><b>${esc(profile.year)}</b></td></tr></table></div></div><div class="cover-foot"><b>${esc(profile.school)}</b><span>${esc(profile.address||profile.region||'')}</span></div></div></section>`}
  function header(title,sem){return `<h1 class="doc-title">${esc(title)}</h1><div class="doc-head"><dl><dt>Satuan Pendidikan</dt><dd>:</dd><dd>${esc(profile.school)}</dd><dt>Mata Pelajaran</dt><dd>:</dd><dd>B. Indonesia</dd><dt>Tahun Pelajaran</dt><dd>:</dd><dd>${esc(profile.year)}</dd></dl><dl><dt>Program</dt><dd>:</dd><dd>${esc(profile.program)}</dd><dt>Kelas / Fase / Semester</dt><dd>:</dd><dd>${className} / A / ${sem==='1'?'I (Ganjil)':'II (Genap)'}</dd><dt>${esc(teacherRole)}</dt><dd>:</dd><dd>${esc(profile.teacher)}</dd></dl></div>`}
