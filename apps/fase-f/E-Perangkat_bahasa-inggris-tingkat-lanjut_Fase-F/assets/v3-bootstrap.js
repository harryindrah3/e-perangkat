(()=>{
  ...
  const safeSet=(key,value)=>{
    try{localStorage.setItem(key,value);return true}
      ...
      if(confirm('Penyimpanan browser penuh. Hapus data B. Inggris TL versi lama agar V23.1 dapat dijalankan? File aplikasi lama di komputer tetap aman.')){
        cleanupOldMath();
  ...
  if(!store||!Array.isArray(store.orders)||!store.orders.length){
    ...
    try{profile={...profile,...JSON.parse(localStorage.getItem('eperangkat.bahasa-inggris-tingkat-lanjut.fasef.v1.profile')||'{}')}}catch(_){}
    let students=[];
    try{students=JSON.parse(localStorage.getItem('eperangkat.bahasa-inggris-tingkat-lanjut.fasef.v1.students')||'[]')}catch(_){}
    const grade=localStorage.getItem('eperangkat.bahasa-inggris-tingkat-lanjut.fasef.v1.grade')||'XI';
  ...
  active.grade=active.grade||'XI';
  safeSet(KEY,JSON.stringify(store));
  safeSet('eperangkat.bahasa-inggris-tingkat-lanjut.fasef.v1.profile',JSON.stringify(lightProfile(active.profile)));
  safeSet('eperangkat.bahasa-inggris-tingkat-lanjut.fasef.v1.students',JSON.stringify(active.students));
  safeSet('eperangkat.bahasa-inggris-tingkat-lanjut.fasef.v1.grade',active.grade);
})();
