(()=>{
  ...
  const KEY='eperangkat.matematika-tingkat-lanjut.fasef.v1.orders';
  ...
  function storageBytes(){
    ...
    for(let index=0;index<localStorage.length;index++){
      ...
      total+=size;
      if(key.startsWith('eperangkat.matematika-tingkat-lanjut.fasef.v1.'))current+=size;
      else if(key.startsWith('eperangkat.matematika-tingkat-lanjut.fasef.v1.legacy.')&&!key.startsWith('eperangkat.matematika-tingkat-lanjut.fasef.v1.'))oldMath+=size;
    }
  ...
  function obsoleteMathKeys(){
    ...
    for(let index=0;index<localStorage.length;index++){
      const key=localStorage.key(index)||'';
      if(key.startsWith('eperangkat.matematika-tingkat-lanjut.fasef.v1.legacy.')&&!key.startsWith('eperangkat.matematika-tingkat-lanjut.fasef.v1.'))keys.push(key);
    }
    return keys;
  }
  ...
  function storageStatusHtml(){
    ...
    return `<div class="storage-status-panel"><div><b>Penggunaan penyimpanan browser</b><span>Total ${formatBytes(size.total)} · V23.1 ${formatBytes(size.current)} · Matematika TL versi lama ${formatBytes(size.oldMath)}</span></div><div><b>${count}</b><span>kunci data Matematika TL versi lama</span></div></div>`;
  }
  ...
  function setStore(store){
    ...
    if(oldKeys.length&&confirm(`Penyimpanan browser penuh. Ditemukan ${oldKeys.length} data Matematika TL dari versi lama. Hapus data browser versi lama agar penyimpanan tersedia? File aplikasi lama tetap aman di komputer.`)){
    ...
    const storageError=new Error('Penyimpanan browser penuh. Gunakan tombol “Bersihkan Data Matematika TL Lama”, lalu ulangi impor.');
  ...
  function saveAndSync(store,order,{reload=false}={}){
    ...
    if(active){
      localStorage.setItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.profile',JSON.stringify(lightProfile(active.profile||D.defaults)));
      localStorage.setItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.students',JSON.stringify(active.students||[]));
      localStorage.setItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.grade',active.grade||'XI');
    }
    refreshOrderBar();
  ...
  function pageHead(title,description,actions=''){
    const store=getStore(),order=activeOrder(store),grade=order?.grade||localStorage.getItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.grade')||'XI';
  ...
  function bindOrderEvents(){
    ...
    $('#exportOrdersBtn').onclick=async()=>{
      ...
      a.download=`backup-eperangkat-Matematika TL-v23-1-${new Date().toISOString().slice(0,10)}.json`;
      a.click();
      ...
      reader.onload=async()=>{
        try{
          ...
          try{
            ...
            const oldKeys=obsoleteMathKeys();
            if(oldKeys.length&&!confirm(`Backup sudah dipadatkan, tetapi ruang masih kurang. Hapus ${oldKeys.length} data browser Matematika TL versi lama lalu lanjutkan impor?`)){
    ...
    $('#cleanupOldMathBtn').onclick=()=>{
      const keys=obsoleteMathKeys();
      if(!keys.length){alert('Tidak ada data Matematika TL versi lama di penyimpanan browser.');return}
      if(!confirm(`Hapus ${keys.length} data browser Matematika TL versi lama? File ZIP dan folder aplikasi lama di komputer tidak ikut terhapus.`))return;
      cleanupOldMathStorage();
      alert('Data browser Matematika TL versi lama berhasil dibersihkan.');
      renderOrders();
    };
  }
  ...
  function renderCalendar(){
    ...
    app.innerHTML=pageHead('Kalender Pendidikan','Kalender Pendidikan: MES 24, MEB 21, HES 112 hari, dan HEB 100 hari menjadi dasar Program Tahunan serta Program Semester Matematika TL.',`<a class="btn primary" href="print.html?grade=${o.grade}&section=calendar&semester=${sem}">Cetak Semester ${sem===1?'I':'II'}</a><a class="btn secondary" href="print.html?grade=${o.grade}&section=calendar&semester=both">Cetak Keduanya</a>`)+`
  ...
  function currentCustomView(){return localStorage.getItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.view')}
  ...
  $$('[data-grade]').forEach(b=>b.addEventListener('click',()=>setTimeout(()=>{
    ...
    const s=getStore(),o=activeOrder(s),g=localStorage.getItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.grade')||b.dataset.grade;if(o&&o.grade!==g){const before=snapshot(o);o.grade=g;history(o,'Kelas pesanan diubah',`Kelas diubah menjadi ${g}.`,before);setStore(s)}
  },0)));
  ...
  $('#quickPrint')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openPrint(`print.html?grade=${localStorage.getItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.grade')||'XI'}&section=all`,'auto')},true);
  ...
  window.addEventListener('eperangkat:students-updated',event=>{
    ...
    try{o.students=JSON.parse(localStorage.getItem('eperangkat.matematika-tingkat-lanjut.fasef.v1.students')||'[]')}
    catch(_){o.students=[]}
