(()=>{
  ...
  const KEY='eperangkat.matematika.fasef.v1.orders';
  ...
  function storageBytes(){
    ...
    for(let index=0;index<localStorage.length;index++){
      ...
      total+=size;
      if(key.startsWith('eperangkat.matematika.fasef.v1.'))current+=size;
      else if(key.startsWith('eperangkat.matematika.fasef.v1.legacy.')&&!key.startsWith('eperangkat.matematika.fasef.v1.'))oldMath+=size;
    }
  ...
  function obsoleteMathKeys(){
    ...
    for(let index=0;index<localStorage.length;index++){
      const key=localStorage.key(index)||'';
      if(key.startsWith('eperangkat.matematika.fasef.v1.legacy.')&&!key.startsWith('eperangkat.matematika.fasef.v1.'))keys.push(key);
    }
    return keys;
  }
  ...
  function saveAndSync(store,order,{reload=false}={}){
    ...
    if(active){
      localStorage.setItem('eperangkat.matematika.fasef.v1.profile',JSON.stringify(lightProfile(active.profile||D.defaults)));
      localStorage.setItem('eperangkat.matematika.fasef.v1.students',JSON.stringify(active.students||[]));
      localStorage.setItem('eperangkat.matematika.fasef.v1.grade',active.grade||'XI');
    }
    refreshOrderBar();
  ...
  function pageHead(title,description,actions=''){
    const store=getStore(),order=activeOrder(store),grade=order?.grade||localStorage.getItem('eperangkat.matematika.fasef.v1.grade')||'XI';
  ...
  function currentCustomView(){return localStorage.getItem('eperangkat.matematika.fasef.v1.view')}
  ...
  $$('[data-grade]').forEach(b=>b.addEventListener('click',()=>setTimeout(()=>{
    ...
    const s=getStore(),o=activeOrder(s),g=localStorage.getItem('eperangkat.matematika.fasef.v1.grade')||b.dataset.grade;if(o&&o.grade!==g){const before=snapshot(o);o.grade=g;history(o,'Kelas pesanan diubah',`Kelas diubah menjadi ${g}.`,before);setStore(s)}
  },0)));
  ...
  $('#quickPrint')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openPrint(`print.html?grade=${localStorage.getItem('eperangkat.matematika.fasef.v1.grade')||'XI'}&section=all`,'auto')},true);
  ...
  window.addEventListener('eperangkat:students-updated',event=>{
    ...
    try{o.students=JSON.parse(localStorage.getItem('eperangkat.matematika.fasef.v1.students')||'[]')}
    catch(_){o.students=[]}
