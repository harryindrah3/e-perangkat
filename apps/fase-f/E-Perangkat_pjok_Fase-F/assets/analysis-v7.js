(()=>{
  ...
  const ORDER_KEY='eperangkat.pjok.fasef.v1.orders';
  const VIEW_KEY='eperangkat.pjok.fasef.v1.view';
  const GRADE_KEY='eperangkat.pjok.fasef.v1.grade';
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>[...r.querySelectorAll(s)];
  const clone=x=>JSON.parse(JSON.stringify(x));
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const gradeNumber={X:10,XI:11,XII:12};
  let currentSemester=localStorage.getItem('eperangkat.pjok.fasef.v1.analysis.semester')||'1';
  let draftOrder=null;
  ...
  function renderAnalysis(){
    ...
    localStorage.setItem('eperangkat.pjok.fasef.v1.analysis.semester',currentSemester);
  ...
  function bindAnalysisEvents(){
    $$('[data-analysis-semester]').forEach(btn=>btn.onclick=()=>{currentSemester=btn.dataset.analysisSemester;localStorage.setItem('eperangkat.pjok.fasef.v1.analysis.semester',currentSemester);renderAnalysis()});
  ...
  function persistDraft(action,detail){
    ...
    setStore(store);
    if(store.activeId===live.id)localStorage.setItem('eperangkat.pjok.fasef.v1.students',JSON.stringify(live.students));
  ...
  function exportCsv(){
    ...
    const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([text],{type:'text/csv;charset=utf-8'}));a.download=`Analisis_Nilai_pjok_Kelas_${grade()}_Semester_${currentSemester}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  }
