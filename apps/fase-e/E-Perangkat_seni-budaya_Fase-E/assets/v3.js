(()=>{
  ...
  const KEY='eperangkat.seni-budaya.fasee.v1.orders';
  ...
  function storageBytes(){
    ...
    for(let index=0;index<localStorage.length;index++){
      ...
      total+=size;
      if(key.startsWith('eperangkat.seni-budaya.fasee.v1.'))current+=size;
      else if(key.startsWith('eperangkat.seni-budaya.fasee.v1.legacy.')&&!key.startsWith('eperangkat.seni-budaya.fasee.v1.'))oldMath+=size;
    }
  ...
  function obsoleteMathKeys(){
    ...
    for(let index=0;index<localStorage.length;index++){
      const key=localStorage.key(index)||'';
      if(key.startsWith('eperangkat.seni-budaya.fasee.v1.legacy.')&&!key.startsWith('eperangkat.seni-budaya.fasee.v1.'))keys.push(key);
    }
    return keys;
  }
  ...
  function saveAndSync(store,order,{reload=false}={}){
    ...
    if(active){
      localStorage.setItem('eperangkat.seni-budaya.fasee.v1.profile',JSON.stringify(lightProfile(active.profile||D.defaults)));
      localStorage.setItem('eperangkat.seni-budaya.fasee.v1.students',JSON.stringify(active.students||[]));
      localStorage.setItem('eperangkat.seni-budaya.fasee.v1.grade',active.grade||'X');
    }
    refreshOrderBar();
  ...
  function pageHead(title,description,actions=''){
    const store=getStore(),order=activeOrder(store),grade=order?.grade||localStorage.getItem('eperangkat.seni-budaya.fasee.v1.grade')||'X';
  ...
  function currentCustomView(){return localStorage.getItem('eperangkat.seni-budaya.fasee.v1.view')}
  ...
  $$('[data-grade]').forEach(b=>b.addEventListener('click',()=>setTimeout(()=>{
    ...
    const s=getStore(),o=activeOrder(s),g=localStorage.getItem('eperangkat.seni-budaya.fasee.v1.grade')||b.dataset.grade;if(o&&o.grade!==g){const before=snapshot(o);o.grade=g;history(o,'Kelas pesanan diubah',`Kelas diubah menjadi ${g}.`,before);setStore(s)}
  },0)));
  ...
  $('#quickPrint')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openPrint(`print.html?grade=${localStorage.getItem('eperangkat.seni-budaya.fasee.v1.grade')||'X'}&section=all`,'auto')},true);
  ...
  document.addEventListener('click',e=>{
    if(e.target.id==='saveProfile')setTimeout(()=>{const s=getStore(),o=activeOrder(s);if(!o)return;let p={...D.defaults};try{p={...p,...JSON.parse(localStorage.getItem('eperangkat.seni-budaya.fasee.v1.profile')||'{}')}}catch(_){}o.profile=p;o.school=p.school;history(o,'Profil satuan pendidikan diperbarui','Identitas dokumen disimpan melalui menu Profil.',beforeProfileSnapshot);setStore(s);refreshOrderBar()},20);
    if(e.target.id==='saveStudents')setTimeout(()=>{const s=getStore(),o=activeOrder(s);if(!o)return;const before=snapshot(o);try{o.students=JSON.parse(localStorage.getItem('eperangkat.seni-budaya.fasee.v1.students')||'[]')}catch(_){o.students=[]}history(o,'Data peserta dan nilai diperbarui',`${o.students.length} peserta tersimpan.`,before);setStore(s)},20);
  });

  // Tombol profil diarahkan ke profil milik pesanan aktif, bukan profil global terpisah.
  $('#profileBtn')?.addEventListener('click',e=>{
    e.preventDefault();e.stopImmediatePropagation();
    localStorage.setItem('eperangkat.seni-budaya.fasee.v1.view','orders');
  ...
  window.addEventListener('eperangkat:students-updated',event=>{
    ...
    try{o.students=JSON.parse(localStorage.getItem('eperangkat.seni-budaya.fasee.v1.students')||'[]')}
    catch(_){o.students=[]}
