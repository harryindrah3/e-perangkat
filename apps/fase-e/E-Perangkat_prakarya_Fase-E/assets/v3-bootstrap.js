(()=>{
  ...
  if(!store||!Array.isArray(store.orders)||!store.orders.length){
    ...
    try{profile={...profile,...JSON.parse(localStorage.getItem('eperangkat.prakarya.fasee.v1.profile')||'{}')}}catch(_){}
    let students=[];
    try{students=JSON.parse(localStorage.getItem('eperangkat.prakarya.fasee.v1.students')||'[]')}catch(_){}
    const grade=localStorage.getItem('eperangkat.prakarya.fasee.v1.grade')||'X';
  ...
  safeSet(KEY,JSON.stringify(store));
  safeSet('eperangkat.prakarya.fasee.v1.profile',JSON.stringify(lightProfile(active.profile)));
  safeSet('eperangkat.prakarya.fasee.v1.students',JSON.stringify(active.students));
  safeSet('eperangkat.prakarya.fasee.v1.grade',active.grade);
})();
