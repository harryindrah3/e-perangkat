(()=>{
  ...
  const PROFILE='eperangkat.pendidikan-agama-islam.fasee.v1.profile';
  const grade=params.get('grade')||localStorage.getItem('eperangkat.pendidikan-agama-islam.fasee.v1.grade')||'X';
  ...
  function cover(){
    return `<section class="page portrait cover schedule-cover"><div class="page-inner cover-content"><div class="cover-kicker">KURIKULUM MERDEKA · PEMBELAJARAN MENDALAM</div><div class="cover-center"><small>E-PERANGKAT PEMBELAJARAN</small><h1>JADWAL MENGAJAR</h1><h2>PAI-BP · Kelas ${esc(grade)} · Fase E</h2><div class="accent-line"></div><div class="identity-box"><table><tr><td>Satuan Pendidikan</td><td>:</td><td><b>${esc(P.school)}</b></td></tr><tr><td>Mata Pelajaran</td><td>:</td><td><b>PAI-BP</b></td></tr><tr><td>${esc(teacherRole)}</td><td>:</td><td><b>${esc(P.teacher)}</b></td></tr><tr><td>Tahun Pelajaran</td><td>:</td><td><b>${esc(P.year)}</b></td></tr></table></div></div><div class="cover-foot"><b>${esc(P.school)}</b><span>${esc(P.address||P.region||'')}</span></div></div></section>`;
  }
  function page(sem){
