(()=>{
  ...
  const PROFILE_KEY='eperangkat.pendidikan-agama-islam.fasec.v1.profile';
  ...
  function rowHtml(row,index){
    return `<tr data-schedule-index="${index}">
      ...
      <td><input data-field="className" value="${esc(row.className)}" placeholder="V"></td>
  ...
  function render(){
    const app=$('#app');
    if(!app)return;
    const p=profile(),grade=localStorage.getItem(GRADE_KEY)||'V';
    ...
    app.innerHTML=`<div class="page-head"><div><span class="eyebrow">Kelas ${esc(grade)} · Fase C</span>
    ...
    $('#resetScheduleExample').onclick=()=>{
      ...
      rows.push(
        {semester:'1',className:'V',day:'SENIN',time:'08.00 – 10.15',hours:3},
        {semester:'2',className:'V',day:'SENIN',time:'08.00 – 10.15',hours:3}
      );
