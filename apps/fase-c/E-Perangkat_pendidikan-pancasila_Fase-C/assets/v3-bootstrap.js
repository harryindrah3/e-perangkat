(()=>{
  ...
  if(!store||!Array.isArray(store.orders)||!store.orders.length){
    ...
    try{profile={...profile,...JSON.parse(localStorage.getItem('eperangkat.pendidikan-pancasila.fasec.v1.profile')||'{}')}}catch(_){}
    let students=[];
    try{students=JSON.parse(localStorage.getItem('eperangkat.pendidikan-pancasila.fasec.v1.students')||'[]')}catch(_){}
    const grade=localStorage.getItem('eperangkat.pendidikan-pancasila.fasec.v1.grade')||'V';
  ...
  safeSet(KEY,JSON.stringify(store));
  safeSet('eperangkat.pendidikan-pancasila.fasec.v1.profile',JSON.stringify(lightProfile(active.profile)));
  safeSet('eperangkat.pendidikan-pancasila.fasec.v1.students',JSON.stringify(active.students));
  safeSet('eperangkat.pendidikan-pancasila.fasec.v1.grade',active.grade);
})();
